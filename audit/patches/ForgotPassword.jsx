/**
 * FORGOT PASSWORD COMPONENT - React Frontend
 * 
 * Features:
 * - Email input → OTP input → New password flow
...
export default function ForgotPassword({ onBack, onSuccess }) {
  ...
  const handleRequestReset = async (e) => {
    ...
    try {
      const res = await resetApi.requestReset(email);
  ...
  const handleVerifyOtp = async (e) => {
    ...
    try {
      const res = await resetApi.verifyOtp(email, otp);
  ...
  const handleResetPassword = async (e) => {
    ...
    try {
      const res = await resetApi.resetPassword(email, resetToken, newPassword);
